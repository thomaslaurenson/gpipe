...
# Do not edit manually. Re-generate with: go run github.com/thomaslaurenson/gpipe generate --repo {{.GithubRepo}} --version {{.Version}}
...
SUPPORTED_PLATFORMS=({{range .Platforms}}{{.ID}} {{end}})
_PLATFORM_OK=false
...
declare -A ASSET_NAMES=({{- range .Platforms}}
  [{{.ID}}]="{{.AssetName}}"{{- end}}
)
...
# Shell completions: included when enabled in .gpipe.yml
