...
'@
    ...
    # cosign writes progress and "Verified OK" to stderr, so 2>&1 captures it
    # (as ErrorRecords, hence the string casts) and it is replayed only on
    # failure. ErrorActionPreference drops to Continue because under 'Stop'
    # some PowerShell versions treat native stderr as terminating, aborting on
    # cosign's ordinary chatter. Restored in a finally rather than a child
    # scriptblock, which makes PSScriptAnalyzer report $TmpDir unused.
